...
def run_qknn_experiment(run_index, X_train, X_test, y_train, y_test, train_kernel_matrix, test_kernel_matrix):
    ...
    # Randomly select 10 k values for this run
    test_size = X_test.shape[0]
    k_values = np.random.choice(range(1, test_size + 1), size=10, replace=False)
...
def send_mail(file_path, run_index):
    ...
    body = f"The results for IRIS Quantum k-NN Run {run_index} have been completed and are attached to this email."
