...
from mpi4py import MPI  # MPI for distributed execution
...
# Ensure TMPDIR environment variable for Milan nodes
base_dir = os.getenv("TMPDIR", "/nobackup/")  # Distributed file location compatibility
file_path = os.path.join(base_dir, 'Standardized_ARCENE_Dataset.csv')  # Full path to dataset

if rank == 0:
    print(f"Rank {rank}: Loading dataset from {file_path}")
    dataset = pd.read_csv(file_path)
else:
    dataset = None

# Broadcast dataset to all nodes
...
n = 5
if rank == 0:
    print(f"Rank {rank}: Quantum k-NN experiment will run {n} times")
...
def run_qknn_experiment(run_index, X_train, X_test, y_train, y_test, train_kernel_matrix, test_kernel_matrix, timings):
    ...
    k_values = range(1, X_test.shape[0] + 1)
...
# Perform train-test split and compute kernel matrices
if rank == 0:
    print(f"Rank {rank}: Normalizing dataset")
...
if rank == 0:
    print(f"Rank {rank}: Creating feature map")
...
if rank == 0:
    print(f"Rank {rank}: Computing train kernel matrix")
...
if rank == 0:
    print(f"Rank {rank}: Computing test kernel matrix")
